...
BEGIN {
    ...
    our @EXPORT  = qw(
        ...
        *lines *indentation &with_emit &emit &indent &outdent &emit_block &emit_package_block
...
sub emit_package_block(&;$$%) {
    # Emit a full protobuf 'package' block (no {...}, no indentation)
    my ($blk, $prefix, $suffix, %flags) = @_;
    my @inner = &with_emit($blk,$indentation);
    return if $flags{-auto} && !@inner;
    $prefix ||= '';
    $suffix ||= '';
    emit $prefix;
...
#        header_ref("cstdint");
	...
	# 	}
	# 	die "Must be primitive: $base\n";
	# }

    return $primitive_aliases{$base} || $base;
}
